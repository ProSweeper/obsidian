...
| Create a JSON Web Token (JWT) that Includes the User's Data |
| Persist a JWT in the Browser |

## Road Map

1. Setup
2. The Plan - Part 5
3. Discuss Token-Based Authentication
...
This lesson continues to build-out the `mern-infrastructure` project right where the _MERN-Stack Infrastructure - Part 4_ lesson left off.
...
**`git reset --hard origin/sync-14-part-5-starter`**
...
Token-based authentication uses a token (string of characters) used to identify the user sending the request to the server.
...
A [JSON Web Token (JWT)](https://jwt.io/introduction/) can contain a data payload including any data we wish. Typically we include data about the user in the payload so that there's no need to query the database (an expensive operation) for the user every time a request hits the server.  This is way more efficient than with session-based auth.
...
<img src="https://i.imgur.com/sYXkmgW.jpg">
...
> ❗️ Keeping the User model lean is always a good practice. However, it's especially important with JWT-based authentication because the user document will be the data payload included in the JWT and you don't want the JWT to be bigger than it has to be!
...
> 👀 The bcrypt library is available for virtually every programming language.
...
**`git reset --hard origin/sync-15-part-5-user-model`**
...
- Create the JWT.  We'll include a `user` property in the JWT's payload containing the user's document data.
...
<details>
<summary>
❓ In the <code>create</code> controller action, how do we access the data sent by the client in the request?
</summary>
<hr>
...
**Add `async` in front of `function` to designate it as an async function.**
...
We need to install another Node module for creating and verifying JWTs.
...
Let's define one in the **.env** file:

```
DATABASE_URL=mongodb+srv://.....mongodb.net/mern-infrastructure?retryWrites=true&w=majority
SECRET=SEIRocks
```
...
> 👀 There are several ways to specify the expiration of the JWT. Check [the docs](https://www.npmjs.com/package/jsonwebtoken) for more info.
...
#### Congrats on Creating a JSON Web Token!
...
**`git reset --hard origin/sync-16-part-5-finish`**
